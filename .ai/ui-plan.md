...
- **Data Fetching Strategy:**
  - **Initial Load:** On page load, fetch the core trip details and accommodation information from `GET /api/trips/[tripId]`.
  - **Primary Lazy Loading:** The main `Transport`, `Days`, and `Files` sections will be lazy-loaded. Data for each will be fetched from their respective endpoints (`GET /api/trips/[tripId]/transport`, `GET /api/trips/[tripId]/days`, `GET /api/trips/[tripId]/files`) only when the user clicks on the corresponding tab or accordion.
  - **Secondary Lazy Loading:** Within the `Transport` and `Days` lists, each item will have an option (e.g., a button or collapsible section) to view associated files. The file list for a specific transport or day will be fetched on-demand from `GET /api/transport/{transportId}/files` or `GET /api/days/{dayId}/files` when the user interacts with that item. This further minimizes data loading to only what is actively being viewed.
